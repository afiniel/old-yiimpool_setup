...
# Welcome
message_box "Afiniel Yiimpool Setup Installer v4.7.5" \
"Hello! Thank you for choosing Afiniel Yiimp Pool Installer!
...
message_box "Afiniel Yiimpool Setup Installer V4.7.5" \
"\n\nWARNING PLEASE READ THIS BEFORE YOU CONTINUEING THIS INSTALLATION.!
...
if [[ ("$UsingSSH" == "yes") ]]; then
  ...
  echo -e "$RED Please reboot system and log in as$GREEN  ${yiimpadmin} $COL_RESET and type$COL_RESET $GREEN yiimpool$COL_RESET $RED to continue setup...$COL_RESET"
  exit 0
fi
...
if [ -z "${yiimpadmin:-}" ]; then
  DEFAULT_yiimpadmin=yiimpadmin
  input_box "New User Login Name" \
  "Please enter your user login name.
  \n\nUser Name:" \
  ${DEFAULT_yiimpadmin} \
  yiimpadmin
...
if [ -z "${RootPassword:-}" ]; then
  ...
  "Enter your new user password. Or you can use this randomly generated one.
  \n\nUnfortunatley you you cant copy your password. So you have to write it down.
  \n\nUser password:" \
...
dialog --title "Does this information match your input?" \
...
0)
clear
echo -e "$YELLOW Creating your new admin account...$COL_RESET"
...
sudo rm -r $HOME/yiimpool
clear
echo -e "$RED New User is $GREEN Created$COL_RESET $RED...$COL_RESET"
