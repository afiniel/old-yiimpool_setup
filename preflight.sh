...
if [ $TOTAL_PHYSICAL_MEM -lt 1436000 ]; then
  if [ ! -d /vagrant ]; then
    ...
    echo "Your Mining Pool needs more memory (RAM) to function properly."
...
if [ $TOTAL_PHYSICAL_MEM -lt 1436000 ]; then
  echo "WARNING: Your Mining Pool
 has less than 1.5 GB of memory."
...
then
  echo "$RED SWAP FILE NEEDED... creates swap file for the system...$COL_RESET"
...
if [ "$ARCHITECTURE" != "x86_64" ]; then
  if [ -z "$ARM" ]; then
    echo "Afiniel Yiimpool Installer only supports x86_64 and will not work on any other architecture, like ARM or 32 bit OS."
